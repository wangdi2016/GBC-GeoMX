...
#
message("========================================")
message("== Question  4:** HGD vs LGD            ")
message("========================================")

#```
## Contrast 4: HGD vs LGD
#We will now compare HGD vs LGD for CK+ and CK- AOIs. The following formula was used to model differences for a given protein:
#$$ protein \sim IM + (1|Scan\_ID) $$
#We adjust for the multiple sampling of ROI segments per patient with the $Scan\_ID$ variable.
#```{r de_contrast4, message=FALSE, warning=FALSE, echo=TRUE}
dfs_contrast4 <- dfs
#to_keep_contrast4 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("IM","CC"), CK=="negative"))
to_keep_contrast4 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("HGD","LGD")))
dfs_contrast4 <- filter_samples(DFS=dfs_contrast4, samples_to_keep = to_keep_contrast4)
dt3 <- ddply(dfs_contrast4[[3]], .(CancerType), summarize, 
       nPatients=length(unique(Scan_ID)), 
       nAOIs=length(CancerType))
knitr::kable(dt3, format="html")

de_contrast4 <- compute_lmer(d=dfs_contrast4, the_formula='protein_i ~ 1 + fixed + (1 |random)  ', 
                         the_factor="CancerType", 
                         the_random = "Scan_ID",
                         the_baseline="LGD")
de_contrast4 <- de_contrast4 %>% filter(isSingular==0, beta!="(Intercept)")
de_contrast4$FDR <- p.adjust(de_contrast4$pval, method = "BH")

#Proteins to label in plots
label_de_contrast4 <- de_contrast4[de_contrast4$pval<0.05,]$Protein
#```
#
...
#::: {#group_de_contrast4 .tabgroup}
#::::: {.tab}
#<button class="tablinks active" onclick="unrolltab(event, 'de_volcano_contrast4', 'group_de_contrast4')">Volcano</button>
#<button class="tablinks" onclick="unrolltab(event, 'de_heatmap_contrast4', 'group_de_contrast4')">Heatmap</button>
#<button class="tablinks" onclick="unrolltab(event, 'de_violin_contrast4_FOI', 'group_de_contrast4')">Violins</button>
#::::::: {#de_volcano_contrast4 .tabcontent style="display: block"}
#```{r de_volcano_contrast4,  fig.width=8, fig.height=6}
de_results_contrast4 <- v_plot2(df= de_contrast4,
        to_label=label_de_contrast4, 
        label_color="black", 
        fc_threshold = 0.50,
        to_lowlight = c(),
        ...
        the_name="Contrast 4 HGD vs LGD",
        left="LGD", right="HGD",
        sizes=medians %>% dplyr::select(protein, Median)
        )
de_results_contrast4[[1]]
#```
#
#:::::::
#::::::: {#de_heatmap_contrast4 .tabcontent}
#```{r de_heatmap_contrast4,  echo=TRUE, message=FALSE, warning=FALSE, collapse=TRUE, fig.width=10, fig.height=10, eval = TRUE}
heatmap_contrast4_features <- de_contrast4
heatmap_contrast4_features <- heatmap_contrast4_features$Protein
heatmap_norm <- as.data.frame(heatmap_dat)
heatmap_dat_contrast4 <- t(scale(t(heatmap_norm[label_de_contrast4, to_keep_contrast4])))
contrast4_de_heatmap <- 
 Heatmap(heatmap_dat_contrast4,
         ...
         column_split = dfs_contrast4[[3]]$CancerType,
         border_gp = gpar(col = "darkgray"),
         show_row_names = FALSE, show_column_names = FALSE,
         right_annotation = 
          rowAnnotation(foo = anno_mark(at = match(label_de_contrast4, rownames(heatmap_dat_contrast4)),
                                        labels = label_de_contrast4)),
         top_annotation = 
          HeatmapAnnotation(df=dfs_contrast4[[3]][factors_of_interest],
                            col = pal_main))
svglite::svglite(filename = file.path(de_dir, "contrast4_de_heatmap.svg"),
                width=10, height=10)
draw(contrast4_de_heatmap, merge_legend = TRUE, heatmap_legend_side = "right",
     annotation_legend_side = "right", adjust_annotation_extension = TRUE)
dev.off()

##
pdf(file.path(de_dir, "contrast4_de_heatmap.pdf"), 
    width = 10, height = 10)

draw(contrast4_de_heatmap, 
     merge_legend = TRUE, 
...
#```
#:::::::
#::::::: {#de_violin_contrast4_FOI .tabcontent}
#<!-- Make a series of violin plots of the label_de7 features. -->
#```{r de_violin_contrast4_FOI, eval = TRUE, echo = TRUE, fig.width=14, fig.height=10}
contrast_factor <- "CancerType"
comparison <- "HGD vs LGD"
p_adjust_class <- "FDR"
p_adjust_method <- "FDR"
violin_df <- cbind(dfs_contrast4[[3]]%>% dplyr::select(eval(contrast_factor)),
                   dfs_contrast4[[2]])
violin_df <- violin_df %>% tidyr::pivot_longer(cols=-1, names_to = "Protein", values_to = "Expression")
colnames(violin_df)[1] <- "contrast_factor"
violin_df <- violin_df %>% filter(Protein %in% label_de_contrast4)

# Format summary stats
violin_p_df <- de_contrast4 %>% filter(isSingular==0, beta!="(Intercept)")
...
ggsave(p, filename = file.path(de_dir, "contrast4_violins.svg"), width=14, height=10)
ggsave(p, filename = file.path(de_dir, "contrast4_violins.pdf"), width=14, height=10)
p
...
#```{r dt results contrast4}
dt_params$buttons <-   list(list(extend = "copy"),
                       list(extend = "csv", filename = "contrast4"),
                       list(extend = "excel", filename = "contrast4"))
dt_contrast4 <- de_contrast4
DT::datatable(
  dt_contrast4[, c(1,2,6, 9)] %>% arrange(pval),
