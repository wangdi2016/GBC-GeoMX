...
# 
message("========================================")
message("== Question  3:** LGD vs IM             ")
message("========================================")

#```
## Contrast 3: LGD vs IM
#We will now compare LGD vs IM for CK+ and CK- AOIs. The following formula was used to model differences for a given protein:
...
dfs_contrast3 <- dfs
#to_keep_contrast3 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("IM","CC"), CK=="negative"))
to_keep_contrast3 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("LGD","IM")))
dfs_contrast3 <- filter_samples(DFS=dfs_contrast3, samples_to_keep = to_keep_contrast3)
dt3 <- ddply(dfs_contrast3[[3]], .(CancerType), summarize, 
...
contrast3_de_heatmap <- 
 Heatmap(heatmap_dat_contrast3,
         ...
         column_split = dfs_contrast3[[3]]$CancerType,
...
##
pdf(file.path(de_dir, "contrast3_de_heatmap.pdf"), 
    width = 10, height = 10)

draw(contrast3_de_heatmap, 
     merge_legend = TRUE, 
...
#```{r de_violin_contrast3_FOI, eval = TRUE, echo = TRUE, fig.width=14, fig.height=10}
contrast_factor <- "CancerType"
comparison <- "LGD vs IM"
p_adjust_class <- "FDR"
p_adjust_method <- "FDR"
...
ggsave(p, filename = file.path(de_dir, "contrast3_violins.svg"), width=14, height=10)
ggsave(p, filename = file.path(de_dir, "contrast3_violins.pdf"), width=14, height=10)
p
