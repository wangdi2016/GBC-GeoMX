...
#
# **Question 5:** IC vs HGD    IC=GBC
# 
message("========================================")
message("== Question  2:** IM vs CC              ")
message("========================================")

#```
## Contrast 2: IM vs CC
#We will now compare IM vs CC CK- AOIs. The following formula was used to model differences for a given protein:
#$$ protein \sim IM + (1|Scan\_ID) $$
#We adjust for the multiple sampling of ROI segments per patient with the $Scan\_ID$ variable.
#```{r de_contrast2, message=FALSE, warning=FALSE, echo=TRUE}
dfs_contrast2 <- dfs
#to_keep_contrast2 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("IM","CC"), CK=="negative"))
to_keep_contrast2 <- row.names(dfs[[3]] %>% filter(CancerType %in% c("IM","CC")))
dfs_contrast2 <- filter_samples(DFS=dfs_contrast2, samples_to_keep = to_keep_contrast2)
dt2 <- ddply(dfs_contrast2[[3]], .(CancerType), summarize, 
       nPatients=length(unique(Scan_ID)), 
       nAOIs=length(CancerType))
knitr::kable(dt2, format="html")

de_contrast2 <- compute_lmer(d=dfs_contrast2, the_formula='protein_i ~ 1 + fixed + (1 |random)  ', 
                         the_factor="CancerType", 
                         the_random = "Scan_ID",
                         the_baseline="CC")
de_contrast2 <- de_contrast2 %>% filter(isSingular==0, beta!="(Intercept)")
de_contrast2$FDR <- p.adjust(de_contrast2$pval, method = "BH")

#Proteins to label in plots
label_de_contrast2 <- de_contrast2[de_contrast2$pval<0.05,]$Protein
#```
#
...
#::: {#group_de_contrast2 .tabgroup}
#::::: {.tab}
#<button class="tablinks active" onclick="unrolltab(event, 'de_volcano_contrast2', 'group_de_contrast2')">Volcano</button>
#<button class="tablinks" onclick="unrolltab(event, 'de_heatmap_contrast2', 'group_de_contrast2')">Heatmap</button>
#<button class="tablinks" onclick="unrolltab(event, 'de_violin_contrast2_FOI', 'group_de_contrast2')">Violins</button>
#::::::: {#de_volcano_contrast2 .tabcontent style="display: block"}
#```{r de_volcano_contrast2,  fig.width=8, fig.height=6}
de_results_contrast2 <- v_plot2(df= de_contrast2,
        to_label=label_de_contrast2, 
        label_color="black", 
        fc_threshold = 0.50,
        to_lowlight = c(),
        ...
        the_name="Contrast 2 IM vs CC",
        left="CC", right="IM",
        sizes=medians %>% dplyr::select(protein, Median)
        )
de_results_contrast2[[1]]
#```
#
#:::::::
#::::::: {#de_heatmap_contrast2 .tabcontent}
#```{r de_heatmap_contrast2,  echo=TRUE, message=FALSE, warning=FALSE, collapse=TRUE, fig.width=10, fig.height=10, eval = TRUE}
heatmap_contrast2_features <- de_contrast2
heatmap_contrast2_features <- heatmap_contrast2_features$Protein
heatmap_norm <- as.data.frame(heatmap_dat)
heatmap_dat_contrast2 <- t(scale(t(heatmap_norm[label_de_contrast2, to_keep_contrast2])))
contrast2_de_heatmap <- 
 Heatmap(heatmap_dat_contrast2,
         ...
         column_split = dfs_contrast2[[3]]$CancerType,
         border_gp = gpar(col = "darkgray"),
         show_row_names = FALSE, show_column_names = FALSE,
         right_annotation = 
          rowAnnotation(foo = anno_mark(at = match(label_de_contrast2, rownames(heatmap_dat_contrast2)),
                                        labels = label_de_contrast2)),
         top_annotation = 
          HeatmapAnnotation(df=dfs_contrast2[[3]][factors_of_interest],
                            col = pal_main))
svglite::svglite(filename = file.path(de_dir, "contrast2_de_heatmap.svg"),
                width=10, height=10)
draw(contrast2_de_heatmap, merge_legend = TRUE, heatmap_legend_side = "right",
     annotation_legend_side = "right", adjust_annotation_extension = TRUE)
dev.off()

##
pdf(file.path(de_dir, "contrast2_de_heatmap.pdf"), 
    width = 10, height = 10)

draw(contrast2_de_heatmap, 
     merge_legend = TRUE, 
...
#```
#:::::::
#::::::: {#de_violin_contrast2_FOI .tabcontent}
#<!-- Make a series of violin plots of the label_de7 features. -->
#```{r de_violin_contrast2_FOI, eval = TRUE, echo = TRUE, fig.width=14, fig.height=10}
contrast_factor <- "CancerType"
comparison <- "IM vs CC"
p_adjust_class <- "FDR"
p_adjust_method <- "FDR"
violin_df <- cbind(dfs_contrast2[[3]]%>% dplyr::select(eval(contrast_factor)),
                   dfs_contrast2[[2]])
violin_df <- violin_df %>% tidyr::pivot_longer(cols=-1, names_to = "Protein", values_to = "Expression")
colnames(violin_df)[1] <- "contrast_factor"
violin_df <- violin_df %>% filter(Protein %in% label_de_contrast2)

# Format summary stats
violin_p_df <- de_contrast2 %>% filter(isSingular==0, beta!="(Intercept)")
...
ggsave(p, filename = file.path(de_dir, "contrast2_violins.svg"), width=14, height=10)
ggsave(p, filename = file.path(de_dir, "contrast2_violins.pdf"), width=14, height=10)
p
...
#```{r dt results contrast2}
dt_params$buttons <-   list(list(extend = "copy"),
                       list(extend = "csv", filename = "contrast2"),
                       list(extend = "excel", filename = "contrast2"))
dt_contrast2 <- de_contrast2
DT::datatable(
  dt_contrast2[, c(1,2,6, 9)] %>% arrange(pval),
